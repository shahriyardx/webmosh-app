...
function DiscussionsInner() {
  ...
  const { data: threads, isLoading } = trpc.discussions.myThreads.useQuery(
    undefined,
  ...
  const select = (taskId: string) => {
    router.replace(`/freelancer/discussions?task=${taskId}`)
  }
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Chat with the admin team about any of your tasks.
          </p>
        </div>
      </div>
       ...
       <div className="flex min-w-0 flex-1 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      >
                        {t.lastMessage
                          ? `${t.lastMessage.fromAdmin ? "Admin: " : "You: "}${t.lastMessage.body}`
          ...
          {activeTaskId ? (
            <>
              <div className="flex items-center gap-3 border-b border-border bg-card px-4 py-3">
                <button
                  type="button"
                  onClick={() => router.replace("/freelancer/discussions")}
                  className="text-muted-foreground hover:text-foreground sm:hidden"
                >
                  <ChevronLeftIcon className="size-5" />
                </button>
                <div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-violet-500/15 text-xs font-semibold text-violet-500">
                  AD
                </div>
                <div className="min-w-0 flex-1">
              ...
              <div className="min-h-0 flex-1">
                <DiscussionChat taskId={activeTaskId} perspective="freelancer" />
              </div>
            </>
          ) : (
            <div className="flex h-full flex-col items-center justify-center gap-2 bg-muted/20 text-center">
              ...
              <p className="max-w-xs text-xs text-muted-foreground">
                Pick any task on the left to start or continue a conversation
                with the admin.
              </p>
            </div>
          )}
        ...
        {activeTaskId && (
          <aside className="hidden w-80 shrink-0 overflow-hidden rounded-2xl border border-border bg-card shadow-sm xl:block">
            <TaskDetailsPanel
              taskId={activeTaskId}
              taskHref={`/freelancer/tasks/${activeTaskId}`}
            />
          </aside>
        )}
      </div>
...
export default function FreelancerDiscussionsPage() {
  return (
