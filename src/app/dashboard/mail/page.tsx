...
type Mail = inferRouterOutputs<AppRouter>["mails"]["list"][number]
...
export default function MailPage() {
  ...
  const { data: mails, isLoading } = trpc.mails.list.useQuery()
  const [selected, setSelected] = useState<Mail | null>(null)

  const markRead = trpc.mails.markRead.useMutation({
    onSuccess: () => {
      utils.mails.list.invalidate()
      utils.mails.unreadCount.invalidate()
    },
  })
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Incoming mail for your company
        </p>
      </div>
                ...
                >
                  <TableCell className={m.read ? "" : "font-semibold"}>
                    <span className="flex items-center gap-2">
                      {!m.read && <span className="size-2 shrink-0 rounded-full bg-amber-500" />}
                      {m.subject}
                    </span>
                  </TableCell>
